...
const ThemeMenu = () => {
    ...
    const setMode = mode => {
        ...
        console.log(dispatch(ThemeActions.setMode(mode.class)))
    }
                            ...
                            mode_settings.map((item, index) => (
                                <li key={index} onClick={() => setMode(item)}>
                                    <div className={
                                            `   mode-list__color 
                                                ${item.background}
                                                ${item.id === currMode ? 'active' : null}
                                             `}>
                                        {item.id === currMode && <i className='bx bx-check' />}
                                    </div>
                                    <span>{item.name}</span>
                                </li>
                            ))
                    ...
                    <ul className='mode-list'>
                        {
                            color_settings.map((item, index) => (
                                <li key={index} onClick={() => setColor(item)}>
                                    ...
                                    > 
                                        {item.id === currColor && <i className='bx bx-check' />}
                                    </div>
                                    <span>{item.name}</span>
                                </li>
                            ))
